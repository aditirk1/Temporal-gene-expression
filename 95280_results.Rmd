...
#load data
raw_counts <- read.table("./data/GSE95280/GSE95280_raw_counts.tsv", header = TRUE, sep = "\t", row.names = 1)
View(raw_counts)
tail(raw_counts)
```
...
ggplot(raw_counts) +
  geom_histogram(aes(x = GSM2501230), stat = "bin", bins = 200) +
  xlim(-5,1000) +
...
raw_counts <- raw_counts[,1:27]
  ...
  plot(cpm_counts[,1],raw_counts[,1],ylim=c(0,50),xlim=c(0,7),xlab="CPM", ylab="Raw Counts")
  abline(v=1.8)
  abline(h=10)
...
```{r}
meta <- data.frame(
  condition = c(rep("501", 12), rep("IGR37", 6), rep("IGR39", 9)),
  time = c(rep(c(0,12,24,48), each=3), rep(c(0,12), each=3), rep(c(0,12,24), each=3)),
  replicate = rep(1:3, 9),
  row.names = paste0("GSM2501", 230:256)
)

meta$state <- ifelse(meta$time == 0, "normoxia", "hypoxia")
write.csv(meta, file="./data/meta_95280.csv")
```
...
expr_matrix <- assay(vsd)
write.table(expr_matrix, file="./data/expr_matrix_95280.txt", sep="\t", quote=F, col.names=NA)
```
...
head(vsd_cor)
# Plot heatmap
pheatmap(vsd_cor, annotation = meta, angle_col = 45, show_rownames = F, show_colnames = F)
```
...
file_name \<- paste0("cluster_mit", i, ".csv") \# Create a file name

write.csv(group_list[[i]], file_name, row.names = FALSE) \# Save each as CSV
