...
#load data
raw_counts <- read.table("./data/GSE53510/GSE53510.tsv", header = TRUE, sep = "\t", row.names = 1)
View(raw_counts)
tail(raw_counts)
```
...
ggplot(raw_counts) +
  geom_histogram(aes(x = GSM1295103), stat = "bin", bins = 200) +
  xlim(-5,1000) +
...
```{r}
sampletype <- factor(c(rep("0_hr_hypoxia",3), rep("8_hr_hypoxia", 3), rep("24_hr_hypoxia", 3), rep("48_hr_hypoxia", 2)),
                     levels = c("0_hr_hypoxia", "8_hr_hypoxia", "24_hr_hypoxia", "48_hr_hypoxia"))
meta <- data.frame(sampletype, row.names = colnames(raw_counts))
meta
write.csv(meta, file="./data/meta_53510.csv")
```
...
#write.table(normalized_counts, file="./data/normalized_counts_53510.txt", sep="\t", quote=F, col.names=NA)
```
...
write.table(expr_matrix, file="./data/expr_matrix_53510.txt", sep="\t", quote=F, col.names=NA)
```
...
sigLRT_genes <- res_LRT_tb %>% dplyr::filter(padj < 0.01, baseMean > 10, abs(log2FoldChange) > 1)
