...
#load data
raw_counts <- read.table("./data/GSE100099/GSE100099.tsv", header = TRUE, sep = "\t", row.names = 1)
View(raw_counts)
tail(raw_counts)
```
...
sample_id <- c(
  paste0("GSM26712", 16:28),  
  "GSM2671285", "GSM2671288",  # Additional rep1 samples
  paste0("GSM26712", 38:50),
  "GSM2671284", "GSM2671286", "GSM2671287",  # rep3
  "GSM2671215", "GSM2671259", "GSM2671237", "GSM2671271", "GSM2671283"  # controls
)

# Create replicate vector
replicate <- c(
  rep(1, 15),  # 13 from 16:28, plus 2 additional
  rep(2, 13),  # 38:50
  rep(3, 3),   # rep3 samples
  rep(1, 2),
  rep(2, 2),
  rep(3,1) # controls
)
...
time <- c(
  1:12, 24, 2.5, 7.5,  # rep1 (15 elements)
  1:12, 24,               # rep2 (13 elements)
  1, 4, 5,                # rep3 (3 elements)
  rep(0, 5)               # controls (5 elements)
)

# Create the condition column
condition <- ifelse(time == 0, "untreated", "10 Gy radiation")

# Create the data frame
meta <- data.frame(
  cell_line = rep("MCF7", length(sample_id)),
  replicate = replicate,
  time = time,
  condition = condition,
  row.names = sample_id
...
print(meta)
write.csv(meta, file="./data/meta_100099.csv")
#filter columns that are not required
raw_counts <- raw_counts[, colnames(raw_counts) %in% sample_id]
```
...
ggplot(raw_counts) +
  geom_histogram(aes(x = GSM2671216), stat = "bin", bins = 200) +
  xlim(-5,1000) +
...
```{r message=FALSE, warning=FALSE}
#mean vs variance
mean_counts <- apply(counts_filtered[, 1:14], 1, mean)
variance_counts <- apply(counts_filtered[, 1:14], 1, var)
...
```{r message=FALSE, warning=FALSE}
# Reorder the rows of 'meta' to match the column order of 'counts_filtered'
meta <- meta[match(colnames(counts_filtered), rownames(meta)), ]
# Check if the rownames of 'meta' and the colnames of 'counts_filtered' match
all(rownames(meta) == colnames(counts_filtered))
...
# write.table(normalized_counts, file="./data/normalized_counts_100099.txt", sep="\t", quote=F, col.names=NA)
```
...
pca_data <- plotPCA(vsd, intgroup = c("time", "replicate", "condition"), returnData = TRUE)
pca_data$replicate <- as.factor(pca_data$replicate)

ggplot(pca_data, aes(x = PC1, y = PC2, group = replicate)) +
  geom_point(aes(color = time, shape = condition), size = 3) +  # Points with color and shape
  geom_line(aes(color = time), linetype = "solid") +  # Solid lines connecting points
  scale_color_gradient(low = "blue", high = "red") +  # Gradient for time points
  labs(title = "PCA",
...
write.table(expr_matrix, file="./data/expr_matrix_100099.txt", sep="\t", quote=F, col.names=NA)
```
...
metafilt$time <- factor(metafilt$time)
```

```{r deg-patterns-plot, fig.width=16, fig.height=10, message = FALSE, warning = FALSE}
clusters <- degPatterns(cluster_vst, metadata = metafilt, time = "time", col = NULL)
...
sigLRT_genes_mit <- res_LRT_tb %>% filter(res_LRT_tb$gene %in% entrez_ids)
